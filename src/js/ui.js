...
class LoadingUtil {
  ...
  show({title = '加载中'} = {title: '加载中'}) {
    if (!this.node) {
      this.node = document.createElement('div')
      this.node.className = 'loading_' + unique
    }
    this.node.innerHTML = createDialog(`
    ...
    `).trim()
    if (!this.showing) document.body.appendChild(this.node)
    this.showing = true
  }

  hide() {
    this.showing && this.node && this.node.remove()
    this.showing = false
  }
...
class ToastUtil {
  constructor() {
    this.node = null  // node 节点
    this.showing = false  // 页面是否展示 node
    this.timeId = null //
  }

  show({title, icon, duration = 1500} = {duration: 1500}) {
    if (!this.node) {
      this.node = document.createElement('div')
      this.node.className = 'toast_' + unique
    }
    this.node.innerHTML = createDialog(`
      <div class="mcx-toast">
        ${
...
class ModalUtil {
  constructor() {
    this.node = null
    this.showing = false
    this.dialogMaskEvent = false
  }

  show({content = '<div></div>'}) {
    if (!this.node) {
      this.node = document.createElement('div')
      this.node.className = 'modal_' + unique
    }
    this.node.innerHTML = createDialog(content).trim()
    document.body.appendChild(this.node)
    if (!this.dialogMaskEvent) {
      const _this = this
      this.node.querySelector('.mcx-dialog-mask').addEventListener('click', function (e) {
        if (this === e.target) _this.hide()
      })
    }
    this.showing = true
  }
